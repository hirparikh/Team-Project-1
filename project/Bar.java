...
{
    private int barWidth = 175; // the width of the color portion of the bar
...
//  The color of the bar AT the breakpoint will be the average color between the safe color and the danger color
     ...
     */
    public Bar(String refText, String unitType, int initValue, int maxValue, Color color)
    {
    ...
    private void newImage()
    {
        ...
        GreenfootImage leftImg = new GreenfootImage(13 * referenceText.length(), (int) fontSize + 4); 
        leftImg.setFont(new Font("OCR A Extended", Font.BOLD, (int) fontSize));
        leftImg.setColor(textColor);
        leftImg.drawString(referenceText, 0, (int) fontSize - 2);
        
        GreenfootImage rightImg = new GreenfootImage(13 * (String.valueOf(value).length() + unitOfMeasure.length()), (int) fontSize); 
        rightImg.setFont(new Font("OCR A Extended", Font.BOLD, (int) fontSize));
        rightImg.setColor(Color.WHITE);
        rightImg.drawString(value + "" + unitOfMeasure, 0, (int) fontSize - 2);
        ...
        barImg.drawImage(rightImg, barImg.getWidth() / 2 - rightImg.getWidth() / 2, barImg.getHeight() / 2 - rightImg.getHeight() / 2);
        
        int maxX = (leftImg.getWidth() > barImg.getWidth()) ? leftImg.getWidth() : barImg.getWidth();
        int maxY = barImg.getHeight() + leftImg.getHeight();
        
        GreenfootImage image = new GreenfootImage(maxX, maxY);
        image.drawImage(barImg, maxX / 2 - barImg.getWidth() / 2, 0);
        image.drawImage(leftImg, maxX / 2 - leftImg.getWidth() / 2, barImg.getHeight());
        setImage(image);
    }
